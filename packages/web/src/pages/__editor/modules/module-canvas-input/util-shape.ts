...
export function shapeArea(vertices: EditableShapeVertex[]): number {
    ...
    for (let i = 0; i < n; ++i) {
        ...
        const p1 = vertices[i].point
        const p2 = vertices[j].point
...
export function findIntersection(
    ...
    for (let i = 0; i < vertices.length; ++i) {
        ...
        if (
            ...
            j !== secondIndex &&
            intersects(
                vertices[firstIndex].point,
                vertices[secondIndex].point,
                vertices[i].point,
                vertices[j].point,
            )
        ) {
...
export function isPointInsideShape(
    entity: EntityWith<EditorComponents, "shape" | "transform">,
    point: Point,
): boolean {
    let isInside = false
...
export function findClosestEdge(
    ...
    for (let i = 0; i < shapes.length; ++i) {
        const shape = shapes[i].get("shape")
        const transform = shapes[i].get("transform")
