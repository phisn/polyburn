...
interface EditorEditorState {
    rendering: RenderingState

    world: EditorWorld

    undos: EditorWorldMutation[]
    redos: EditorWorldMutation[]
}

export interface EditorEditorStore extends EditorEditorState {
    mutate: (mutation: EditorWorldMutation) => void
...
const createInitialEditorEditorState = (): EditorEditorState => {
...
export const useEditorEditorStore = create<EditorEditorStore>((set, get) => ({
    ...createInitialEditorEditorState(),
    mutate: (mutation: EditorWorldMutation) =>
        set((state) => ({
            editorWorld: mutation.redo(state.world),
            undos: [...state.undos, mutation],
            redos: [],
        })),
    undo: () =>
    ...
    canvasToWorld: (x: number, y: number) => {
        ...
        vector.set(
            (x / state.rendering.canvasSize.width) * 2 - 1,
            -(y / state.rendering.canvasSize.height) * 2 + 1,
            -1
        )

        vector.unproject(state.rendering.camera)
