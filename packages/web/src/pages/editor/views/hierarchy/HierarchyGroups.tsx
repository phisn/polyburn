...
export function HierarchyGroups() {
        ...
        () => ({
            ...
            drop: (item: DraggableItem) => {
                console.log(
                    props.group,
                    item,
                    (world.entities[item.entityKey].group ?? "") === props.group,
                )

                if ((world.entities[item.entityKey].group ?? "") === props.group) {
                    return
                }

                updateWorld(world => {
                    console.log("do")
                    world.entities[item.entityKey].group =
                        props.group === "" ? undefined : props.group
                })
            },
            collect: monitor => ({
...
function HierarchyEntryEntity(props: {
    ...
    const setCameraTarget = useEditorStore(x => x.setCameraTarget)
    ...
    function onDoubleClick(event: MouseEvent) {
        setCameraTarget(props.entitiy.transform.point)
        select(props.entityKey, event.shiftKey)
    }
    ...
    return (
        <div
            ...
            className={
                "flex w-full items-center space-x-2 rounded px-2 py-1 transition " +
