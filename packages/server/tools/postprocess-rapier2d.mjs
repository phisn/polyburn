...
function transformScript() {
    ...
    if (head === scriptHead) {
        ...
        console.log('rapier2d wasm bindings already postprocessed')
        return
    }

    const withCustomWasm = [
        scriptHead,
        "import * as _wasm from './rapier_wasm2d_bg.wasm';",
        "let wasm = _wasm;",
...
function transformScriptTypes() {
    ...
    if (fs.existsSync(scriptTypes)) {
        console.log('rapier2d wasm bindings types already postprocessed')
        return
    }
...
console.log('rapier2d wasm bindings postprocessing done')
