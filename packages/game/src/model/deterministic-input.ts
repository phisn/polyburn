...
import { GameInput } from "game/src/model/api"

export function applyDeterministicInput(
    replay: ReplayInputModel,
    onUpdate: (input: GameInput) => void,
) {
    const replayDeltaInputs = decodeDeterministicInput(replay.inputDeterministic)
...
export class InputDeterminism {
    ...
    process(input: GameInput): DeterminsticInput {
        let changeRounded = f16round(input.rotation - this.accumulated)
        if (Math.abs(changeRounded) < InputDeterminism.MINIMAL_CHANGE_TO_CAPTURE) {
            changeRounded = 0
        }
...
export interface DeterminsticInput {
...
export function encodeDeterministicInput(replayFrames: DeterminsticInput[]) {
    const packets = [
...
export function decodeDeterministicInput(bytes: Uint8Array): DeterminsticInput[] {
    ...
    const gameInputs: DeterminsticInput[] = []
...
function packRotations(frames: DeterminsticInput[]): Packet[] {
    enum packDiffType {
        Zero = 0,
        NonZero = 1,
    }
