...
export function isCollisionWithCapture(collision: CollisionMessage, other: Entity<RuntimeComponents>): other is LevelEntity {
    return collision.other?.has(...LevelEntityComponents) &&
        collision.other.components.level.captureCollider.handle === collision.otherCollider.handle &&
