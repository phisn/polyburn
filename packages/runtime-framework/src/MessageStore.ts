...
import { InferTargetComponents, MessageWithTarget, WithoutTarget, WithTarget } from "./MessageWithTarget"
...
export interface MessageStore<Components extends object, Messages extends object> {
    ...
    publishTarget<T extends keyof WithTarget<Messages, Components>>(messageName: T, message: WithTarget<Messages, Components>[T]): this

    collect<T extends keyof Messages>(messageName: T): MessageCollector<Required<Messages>[T]>
    collectTarget<T extends keyof WithTarget<Messages, Components>, K extends (keyof Components & keyof InferTargetComponents<Messages[T]>)[]>(messageName: T, ...components: [...K]): MessageCollector<MessageWithTarget<NarrowProperties<Components, K[number]>> & WithTarget<Required<Messages>, Components>[T]>

    listenTo<T extends keyof Messages>(messageName: T, callback: (message: Required<Messages>[T]) => void): () => void
}
...
export const createMessageStore = <Components extends object, Messages extends object>(): MessageStore<Components, Messages> => {
    const listenerMap = new Map<string, Set<(message: Required<Messages>[keyof Messages]) => void>>()
    const componentListenerMap = new Map<string, Set<(message: MessageWithTarget<Partial<Components>>) => void>>()
    ...
    return {
        ...
        publishTarget<T extends keyof WithTarget<Messages, Components>>(messageName: T, message: WithTarget<Messages, Components>[T]) {
            for (const component of Object.keys(message.target.components)) {
                for (const callback of componentListenerMap.get(`${messageName.toString()}${component}`) ?? []) {
                    callback(message)
                }
            }
        ...
        collectTarget<T extends keyof WithTarget<Messages, Components>, K extends (keyof Components & keyof InferTargetComponents<Messages[T]>)[]>(messageName: T, ...components: [...K]): MessageCollector<MessageWithTarget<NarrowProperties<Components, K[number]>> & WithTarget<Required<Messages>, Components>[T]> {
            ...
            const collector: MessageCollector<MessageWithTarget<NarrowProperties<Components, K[number]>> & WithTarget<Required<Messages>, Components>[T]> = {
                [Symbol.iterator]() {
                    const consumedMessages = messages as (MessageWithTarget<NarrowProperties<Components, K[number]>> & WithTarget<Required<Messages>, Components>[T])[]
                    messages = []
                    return consumedMessages.values()
                },
            }
